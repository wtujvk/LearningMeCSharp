...
namespace wtujvk.LearningMeCSharp.ToolStandard.Messages
{
    ...
    internal class SMSMessageManager : IMessageManager
    {
        ...
        #region Fields
        static string gateway = ConfigManager.Config.Messaging.SMSGateway;

        static string sign_type = ConfigManager.Config.Messaging.SMSSignType;
        static string input_charset = ConfigManager.Config.Messaging.SMSCharset;
        static string key = ConfigManager.Config.Messaging.SMSKey;
        static int itemID = ConfigManager.Config.Messaging.SMSItemID;
        #endregion
        ...
        public int Send(IEnumerable<string> recipients, string subject, string body, Action errorAction = null, Action successAction = null)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(mobile))
                {
                    ...
                    LoggerFactory.Instance.Logger_Info("手机号" + mobile + ",发送结果：" + result);
                    if (result != "1")
                    {
                        if (errorAction != null)
                        {
